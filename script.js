...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async (e) => {
      ...
      try {
        // Enviar a la base de datos SQLite vía backend
        ...
        console.warn('Backend no disponible, funcionando en modo sin conexión:', err);
        // Fallback si el servidor no está corriendo
        if (successBanner) {
          successBanner.style.display = 'flex';
          successBanner.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        }
...
// Obtener contenido dinámico desde SQLite
async function fetchDynamicContent() {
  try {
    ...
    const { settings, services, cases, testimonials } = data;

    // 1. Actualizar Ajustes y WhatsApp
    if (settings) {
      ...
      // Actualizar contadores si vienen de base de datos
    ...
    console.log('✅ Contenido dinámico cargado desde SQLite.');
  } catch (err) {
    // Si el servidor aún no está encendido, el frontend estático base funciona de inmediato
    console.log('ℹ️ Modo autónomo estático activo.');
  }
}
